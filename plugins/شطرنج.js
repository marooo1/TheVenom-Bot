...
const handler = async (m, { conn, args }) => {
  ...
  if (feature === 'حذف') {
    ...
    return conn.reply(m.chat, '『JACK♦️BOT』 *تم حذف اللعبه*', m);
  }

  if (feature === 'صمم') {
    if (gameData) {
      return conn.reply(m.chat, '『JACK♦️BOT』 *اللعبه قائمه بالفعل*', m);
    }
    chessData.gameData = { status: 'waiting', black: null, white: null };
    return conn.reply(m.chat, '『JACK♦️BOT』 *تم تصميم الغرقه*\nفي انتظار اللاعبين للانضمام.', m);
  }

  if (feature === 'ادخل') {
    const senderId = m.sender;
    if (players.includes(senderId)) {
      return conn.reply(m.chat, '『JACK♦️BOT』 *انت لقد قمت بالانضمام بالفعل*', m);
    }
    if (!gameData || gameData.status !== 'waiting') {
      return conn.reply(m.chat, '『JACK♦️BOT』 *لا يوجد لعبه شطرنج في انتظار اللاعبين حاليا*', m);
    }
    if (players.length >= 2) {
      return conn.reply(m.chat, '『JACK♦️BOT』 *اكتمل عدد اللاعبين بالفعل.*\nاللعبه هتبدا لوحدها', m);
    }
    ...
    if (players.length === 2) {
      ...
      return conn.reply(m.chat, `『JACK♦️BOT』 *اللاعبين الي اضمو:*\n${hasJoined.map(playerId => `- @${playerId.split('@')[0]}`).join('\n')}\n\n*الاسود:* @${black.split('@')[0]}\n*الابيض:* @${white.split('@')[0]}\n\nمن فضل استخدم *'شطرنج ابدا'* لبدا اللعبه.`, m, { mentions: hasJoined });
    } else {
      return conn.reply(m.chat, '『JACK♦️BOT』*انت لقد انضممت للعبه الشطرنج.*\nفي انتظار اللاعب الاخر', m);
    }
  }

  if (feature === 'ابدا') {
    if (gameData.status !== 'ready') {
      return conn.reply(m.chat, '『JACK♦️BOT』 *مش عارف ابدا اللعبه. لازم يخش لاعبين الثنين للعب*', m);
    }
    ...
    if (players.length === 2) {
      ...
      const turn = `『JACK♦️BOT』دور: *الابيض* @${gameData.white.split('@')[0]}`;
      ...
      return conn.reply(m.chat, '『JACK♦️BOT』 *انت لقد انضممت الي اللعبه*\nفي انتظار اللاعب الثاني لدخول اللعبه', m);
    }
  }
  ...
  if (args[0] && args[1]) {
    ...
    if (!gameData || gameData.status !== 'playing') {
      return conn.reply(m.chat, '『JACK♦️BOT』 *اللعبه لسه مبدأتش*', m);
    }
    if (currentTurn !== senderId) {
      return conn.reply(m.chat, `『JACK♦️BOT』 *المفروض* ${chessData.currentTurn === gameData.white ? 'الابيض' : 'الاسود'}'هو الي يتحرك*`, m, {
        contextInfo: {
    ...
    if (chess.isCheckmate()) {
      delete conn.chess[key];
      return conn.reply(m.chat, `『JACKK♦️BOT』 *كش مات☠️*\n*اللعبه خلصت.*\n*الكسبان 🎉:* @${m.sender.split('@')[0]}`, m, {
        contextInfo: {
    ...
    try {
      ...
      return conn.reply(m.chat, '『JACK♦️BOT』 *الحركه غير صالحه*', m);
    }
    ...
    const turn = `『JACK♦️BOT』 *دور:* ${currentColor} @${chessData.currentTurn.split('@')[0]}\n\n${chess.getComment() || ''}`;
    const flipParam = senderId === gameData.black ? '' : '&flip=true';
    const flipParam2 = senderId === gameData.black ? '' : '-flip';
    const boardUrl = `https://www.chess.com/dynboard?fen=${encodedFen}&board=graffiti&piece=graffiti&size=3&coordinates=inside${flipParam}`;
    try {
...
بعدها اكتب *شطرنج ادخل* عشان تخش انت وصاحبك اللعبه.
  ...
  return conn.reply(m.chat, '『JACK♦️BOT』“ امر خاطئ. استعمل*"شطرنج مساعده"* عشان تشوف الاوامر شغاله ازاي.', m);
};
